...
import { ArchiveIterator } from '../archive/archive_iterator';
...
import { ArchiveIteratorFromMap } from '../archive/archive_iterator_from_map';
...
async function installPackageByUpload({
  ...
  try {
    ...
    const archiveIterator = new ArchiveIterator(archiveBuffer, contentType);
    let paths: string[];
...
export async function installPackage(args: InstallPackageParams): Promise<InstallResult> {
  ...
  if (args.installSource === 'registry') {
    ...
    const { pkgName } = Registry.splitPkgKey(pkgkey);
    const useStreaming = PACKAGES_FOR_STREAM_INSTALL.includes(pkgName);
...
export async function installCustomPackage(
  ...
  const archiveIterator = new ArchiveIteratorFromMap(assetsMap);
...
export async function installAssetsForInputPackagePolicy(opts: {
  ...
  if (installedPkgWithAssets.installation.version !== pkgInfo.version) {
    ...
    const archiveIterator = new ArchiveIteratorFromMap(pkg.assetsMap);
    packageInstallContext = {
    ...
    const archiveIterator = new ArchiveIteratorFromMap(installedPkgWithAssets.assetsMap);
    packageInstallContext = {
